...
impl Participant {
    ...
    pub fn new(
        ...
        operation_success_prob: f64,
        rx: Receiver<ProtocolMessage>,
        tx: Sender<ProtocolMessage>) -> Participant {
    ...
    pub fn send(&mut self, pm: ProtocolMessage){
        ...
        if x <= self.send_success_prob {
            // TODO: Send success
            self.log.append(pm.mtype, pm.txid.to_string(), pm.senderid.to_string(), pm.opid);
            self.tx.send(pm);
            // true
        } else {
            //false
        }
        trace!("{}::Sent.", self.id_str.clone());
    }
    ...
    ///
    pub fn perform_operation(&mut self) -> bool { //request_option: &Option<ProtocolMessage>

        trace!("{}::Performing operation", self.id_str.clone());
        let x: f64 = random();
        if x <= self.operation_success_prob {
            // TODO: Successful operation
            // *request_option.as_ref().unwrap().mtype = MessageType::ParticipantVoteCommit;
            true
        } else {
    ...
    pub fn protocol(&mut self) {
        ...
        loop {
            let message = &mut self.rx.recv().unwrap();
            match message.mtype {
                MessageType::CoordinatorPropose => {
                    let mut mType = MessageType::ParticipantVoteAbort;
                    if self.perform_operation() {
                        trace!("{}::Voting Commit", self.id_str.clone());
                        mType = MessageType::ParticipantVoteCommit;
                    }
                    let message = ProtocolMessage::instantiate(mType, message.uid, message.txid.to_string(), self.id_str.to_string(), message.opid);
                    self.send(message);
                    trace!("{}::Sent message", self.id_str.clone());
                }
                MessageType::CoordinatorAbort => {
                    // collect stat
                    self.log.append(message.mtype, message.txid.to_string(), message.senderid.to_string(), message.opid);
                    fails += 1;
                }
                MessageType::CoordinatorCommit => {
                    // Collect stats for success
                    self.log.append(message.mtype, message.txid.to_string(), message.senderid.to_string(), message.opid);
                    successes += 1;
                }
